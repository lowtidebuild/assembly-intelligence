 ...
 *
 * ⚠️ `proposalReason` and `mainContent` are currently ALWAYS NULL —
 * MCP does not expose 제안이유/주요내용 in any tool we tried
 * (assembly_bill detail, query_assembly with 5 candidate api_codes).
 * Retained as nullable columns for future (HTML scrape or new MCP API).
 * Lane B Gemini prompts must handle null by using 의안명 + 소관위원회
 * + 대표발의자 as the scoring signal.
 *
