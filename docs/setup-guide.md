...
**프리셋 카드를 클릭하면 같은 화면에 "관련 법률 (선택)" 체크박스가 나타납니다.** 선택한 산업이 가로지르는 법률을 추가로 체크하면, 해당 법률의 키워드가 watchlist에 합쳐집니다.

| 법률 믹스인 | 주요 키워드 예시 | 어떤 산업에 유용한가 |
|---|---|---|
| ☐ 전자상거래법 | 전자상거래, 오픈마켓, 다크패턴, 청약철회 | 이커머스·게임·플랫폼 |
| ☐ 표시·광고법 | 부당광고, 기만적 광고, 비교광고 | 이커머스·게임·광고 |
| ☐ 정보통신망법 | 정보통신망, 본인확인, 스팸, 위치정보 | 보안·게임·플랫폼 |
| ☐ 개인정보보호법 | 개인정보, 가명정보, 프로파일링, 국외이전 | 전산업 공통 |
| ☐ 저작권법 | 저작권, TDM, 2차적저작물, 공정이용 | AI·게임·콘텐츠 |
| ☐ 이스포츠 진흥법 | 이스포츠, 프로게이머, 표준계약서 | 게임·스포츠 |
| ☐ 청소년 보호법 | 셧다운제, 청소년 게임시간, 연령 등급 | 게임·콘텐츠 |
| ☐ 콘텐츠산업진흥법 | 콘텐츠진흥원, 콘텐츠 수출, 문화콘텐츠 | 게임·콘텐츠·미디어 |

**팁**: 게임사라면 `게임` 프리셋 + `전자상거래법` + `정보통신망법` + `저작권법` + `이스포츠 진흥법` 조합이 흔합니다. 화면 하단에 "Pre-filter 키워드: N개 = 기본 M개 + 법률 K개 추가" 실시간 카운트가 나옵니다.
...
### Q: 법안이 5개밖에 안 잡혀요
**A**: 키워드가 너무 좁을 수 있습니다. 설정 위저드에서 키워드를 추가하거나 Step 1의 **관련 법률 믹스인**을 추가로 체크해보세요. 예를 들어 게임사라면 `게임산업법`만으로는 부족하고 `전자상거래법`·`정보통신망법`·`이스포츠 진흥법`을 함께 켜면 교차 법률 발의안까지 잡힙니다. 일반적으로 총 효과 키워드 30-50개가 적정.
